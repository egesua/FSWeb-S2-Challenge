...
function cumlelereDonustur(cumleler, seperator = ",") {
  const newArray = cumleler.map((item) => {
...
function paragrafOlustur(cumleler, cumleKur, callback) {
    let newArray = cumlelereDonustur(cumleler, " ");
    let answer = cumleKur(newArray[1], newArray[3], newArray[5], newArray[7], newArray[9]);
    return answer;
}
...
function emojileriDonustur(message, emojiler) {
  for (let key in emojiler) {
    message = message.replaceAll(key.toLowerCase(), emojiler[key])
    message = message.replaceAll(key.toUpperCase(), emojiler[key])
  }
  return message;
}
